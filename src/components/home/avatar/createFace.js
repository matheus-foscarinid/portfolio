...
const AVATAR_EYES = {
  centers: [new Vector3(-0.0235, 0.834, 0.111), new Vector3(0.0425, 0.8365, 0.113)],
  // the depth keeps the lid off the glasses frame
  halfSize: new Vector3(0.0195, 0.0085, 0.013),
  // sampled from the texture just under the eye
  lidColors: [new Color('#b3856c'), new Color('#b3856c')],
};
...
const createEyelidShader = (eyeCount) => /* glsl */ `
  varying vec3 vRestPosition;
  uniform float uBlink;
  uniform float uHappyEyes;
  uniform vec3 uEyeCenters[${eyeCount}];
  uniform vec3 uEyeHalfSize;
  uniform vec3 uLidColors[${eyeCount}];
  uniform vec3 uLashColor;

  vec3 applyEyelid(vec3 color, vec3 center, vec3 lidColor) {
    ...
    vec3 lid = mix(color, lidColor, cover);
    return mix(lid, uLashColor, lashes * 0.85);
  }

  vec3 applyEyelids(vec3 color) {
    for (int index = 0; index < ${eyeCount}; index++) color = applyEyelid(color, uEyeCenters[index], uLidColors[index]);
    return color;
  }
`;

const patchMaterial = (material, uniforms, eyeCount) => {
  material.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace('#include <common>', `#include <common>\n${createEyelidShader(eyeCount)}`)
      .replace('#include <map_fragment>', '#include <map_fragment>\ndiffuseColor.rgb = applyEyelids(diffuseColor.rgb);');
  };
  material.needsUpdate = true;
};

const createUniforms = ({ centers, halfSize, lidColors }) => ({
  uBlink: { value: 0 },
  uHappyEyes: { value: 0 },
  uEyeCenters: { value: centers },
  uEyeHalfSize: { value: halfSize },
  uLidColors: { value: lidColors },
...
export const createFace = (material, eyes = AVATAR_EYES) => {
  const uniforms = createUniforms(eyes);
  patchMaterial(material, uniforms, eyes.centers.length);
